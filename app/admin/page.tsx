...
export default function AdminPanel() {
  ...
  const activeCount = bugTypes.filter(b => isActive(b.id, selectedCustomer.id)).length
        ...
        <div className="w-72 min-h-screen bg-gray-900 border-r border-gray-800 p-4">
          ...
          <div className="space-y-2">
            {customers.map(customer => {
              const activebugs = bugTypes.filter(b => isActive(b.id, customer.id)).length
              return (
                <button
                  key={customer.id}
          ...
          <div className="mt-6 pt-6 border-t border-gray-800">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 bg-red-900 text-red-400 rounded text-xs font-bold">P1</span>
                <span className="text-gray-500 text-xs">Critical — system down</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 bg-orange-900 text-orange-400 rounded text-xs font-bold">P2</span>
                <span className="text-gray-500 text-xs">High — major feature broken</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 bg-blue-900 text-blue-400 rounded text-xs font-bold">P3</span>
                <span className="text-gray-500 text-xs">Medium — workaround available</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 p-8">
          ...
          {saved && (
            <div className="bg-green-900 border border-green-700 rounded-xl p-4 mb-6 flex items-center gap-3">
              ...
              <p className="text-green-300 font-medium">Bug states saved — {selectedCustomer.name} will now experience the active issues when logging into Doxaflow</p>
            </div>
          )}
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="bg-cyan-400 hover:bg-cyan-300 text-gray-950 font-semibold rounded-lg px-5 py-2 text-sm transition"
              >
                Save Changes
              </button>
            </div>
                  ...
                  {active && (
                    <div className="mt-3 pt-3 border-t border-red-800">
                      <p className="text-red-400 text-xs font-medium">
                        🔴 ACTIVE — {selectedCustomer.name} will experience this issue when logged into Doxaflow. This will generate a realistic {bug.severity} support ticket.
                      </p>
                    </div>
                  )}
                </div>
